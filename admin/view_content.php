...
if(isset($_POST['delete_video'])){
   ...
   $delete_video_thumb = $conn->prepare("SELECT thumb FROM `contents` WHERE id = ? LIMIT 1");
   $delete_video_thumb->execute([$delete_id]);
   $fetch_thumb = $delete_video_thumb->fetch(PDO::FETCH_ASSOC);
   unlink('../uploaded_files/'.$fetch_thumb['thumb']);

   $delete_video = $conn->prepare("SELECT video FROM `contents` WHERE id = ? LIMIT 1");
...
<section class="view-content">
   ...
   <?php
      $select_content = $conn->prepare("SELECT * FROM `contents` WHERE id = ? AND tutor_id = ?");
