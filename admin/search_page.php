...
if(isset($_POST['delete_video'])){
   ...
   $verify_video = $conn->prepare("SELECT * FROM `contents` WHERE id = ? LIMIT 1");
   $verify_video->execute([$delete_id]);
   if($verify_video->rowCount() > 0){
      $delete_video_thumb = $conn->prepare("SELECT * FROM `contents` WHERE id = ? LIMIT 1");
      $delete_video_thumb->execute([$delete_id]);
      $fetch_thumb = $delete_video_thumb->fetch(PDO::FETCH_ASSOC);
      unlink('../uploaded_files/'.$fetch_thumb['thumb']);
      $delete_video = $conn->prepare("SELECT * FROM `contents` WHERE id = ? LIMIT 1");
...
<section class="contents">
   ...
   <?php
      ...
      $search = $_POST['search'];
      $select_videos = $conn->prepare("SELECT * FROM `contents` WHERE title LIKE '%{$search}%' AND tutor_id = ? ORDER BY date DESC");
   ...
   <div class="box-container">
      ...
      if(isset($_POST['search']) or isset($_POST['search_btn'])){
         ...
         while($fetch_playlist = $select_playlist->fetch(PDO::FETCH_ASSOC)){
            ...
            $count_videos = $conn->prepare("SELECT * FROM `contents` WHERE playlist_id = ?");
